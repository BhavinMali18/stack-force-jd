 ...
 * STATUS: DISABLED
 *
 * The Gemini AI scoring flow is commented out.
 * All resume scoring runs through the rule-based pipeline:
 *   → skillMatcher.js (computeMatchScore)
 *   → Must-have: 60% weight | Nice-to-have: 40% weight
 *   → Score capped at 40 if any must-have skill is missing
 *
 * To re-enable AI scoring:
 *   1. Uncomment the code below
 *   2. Fix the model name: 'gemini-3.5-flash' → 'gemini-2.5-flash'
 *   3. Set GEMINI_API_KEY in server/.env
 *   4. Re-enable the import in candidate.controller.js
 */
...
// ── Disabled stubs — throw immediately if accidentally called ──
const analyzeResumeWithAI = async () => {
  throw new Error('[aiService] AI scoring is disabled. Use computeMatchScore() instead.');
};

const parseJDWithAI = async () => {
  throw new Error('[aiService] AI JD parsing is disabled.');
};
